...
def fdmnes_calculator_mpi(js,ncores,host_ind):
    ...
    try:
        ...
        exe_path = '/gpfs/projects/FrenkelGroup/softwares/parallel_fdmnes'
    ...
    try:
        ...
        print(host)
        for e in exe_list:
            _ = subprocess.run(['HOST_NUM_FOR_MUMPS=5  bash %s/%s -np %d -hostfile %s --host %s>> fdmnes.out ' %(exe_path,e,ncores,"hostfile",host)],shell=True) #--hostfile %s --host %s 
...
site = int(cif_files[ind].split('.')[0].split('site_')[1].split('_cn')[0])+1
...
with open(path+'fdmnes.inp') as fi:
    fdmnesinp = fi.readlines()
fdmnesinp[12]="Absorber\n"
fdmnesinp[13]=str(site)+'\n'
...
with open('output/'+cif_files[ind].split('/')[1].split('.')[0]+'.json', 'w') as f:
    json.dump(js_out, f)
