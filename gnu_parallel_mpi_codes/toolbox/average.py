...
def load_files(filename,i):
    ...
    site=int(filename[i].split("site_")[1].split('_')[0])
    n_sites=int(filename[i].split('num_')[1].split('_')[0])
    cn=int(filename[i].split('cn_')[1].split('.')[0])
    return particle,Energy,mu,site,n_sites,cn
